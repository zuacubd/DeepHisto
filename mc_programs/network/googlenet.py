...
#import tensorflow.python.frameworks as ops

def get_inception_layer( inputs, conv11_size, conv33_11_size, conv33_size,
                         conv55_11_size, conv55_size, pool11_size ):
    with tf.variable_scope("conv_1x1"):
        conv11 = tf.contrib.layers.conv2d( inputs, conv11_size, [ 1, 1 ] )
    with tf.variable_scope("conv_3x3"):
        conv33_11 = tf.contrib.layers.conv2d( inputs, conv33_11_size, [ 1, 1 ] )
        conv33 = tf.contrib.layers.conv2d( conv33_11, conv33_size, [ 3, 3 ] )
    with tf.variable_scope("conv_5x5"):
        conv55_11 = tf.contrib.layers.conv2d( inputs, conv55_11_size, [ 1, 1 ] )
        conv55 = tf.contrib.layers.conv2d( conv55_11, conv55_size, [ 5, 5 ] )
    with tf.variable_scope("pool_proj"):
        pool_proj = tf.contrib.layers.max_pool2d( inputs, [ 3, 3 ], stride = 1 )
        pool11 = tf.contrib.layers.conv2d( pool_proj, pool11_size, [ 1, 1 ] )
...
def aux_logit_layer( inputs, num_classes, is_training ):
    with tf.variable_scope("pool2d"):
        pooled = tf.contrib.layers.avg_pool2d(inputs, [ 5, 5 ], stride = 3 )
    with tf.variable_scope("conv11"):
        conv11 = tf.contrib.layers.conv2d( pooled, 128, [1, 1] )
    with tf.variable_scope("flatten"):
        flat = tf.reshape( conv11, [-1, 2048] )
    with tf.variable_scope("fc"):
        fc = tf.contrib.layers.fully_connected( flat, 1024, activation_fn=None )
    with tf.variable_scope("drop"):
        drop = tf.contrib.layers.dropout( fc, 0.3, is_training = is_training )
    with tf.variable_scope( "linear" ):
        linear = tf.contrib.layers.fully_connected( drop, num_classes, activation_fn=None )
...
def googlenet(inputs,
    ...
    """Implementation of googlenet (https://arxiv.org/pdf/1409.4842.pdf)
    """

    end_points = {}
    with tf.name_scope(scope, "googlenet", [inputs]):
        with tf.contrib.framework.arg_scope( [tf.contrib.layers.max_pool2d ], padding = 'SAME' ):
            end_points['conv0'] = tf.contrib.layers.conv2d( inputs, 64, [ 7, 7 ], stride = 2, scope = 'conv0' )
            end_points['pool0'] = tf.contrib.layers.max_pool2d( end_points['conv0'], [3, 3], scope='pool0')
            end_points['conv1_a'] = tf.contrib.layers.conv2d( end_points['pool0'], 64, [ 1, 1 ], scope = 'conv1_a' )
            end_points['conv1_b'] = tf.contrib.layers.conv2d( end_points['conv1_a'], 192, [ 3, 3 ], scope = 'conv1_b' )
            end_points['pool1'] = tf.contrib.layers.max_pool2d( end_points['conv1_b'], [ 3, 3 ], scope='pool1')
            ...
            end_points['pool2'] = tf.contrib.layers.max_pool2d(end_points['inception_3b'], [ 3, 3 ], scope='pool2')
            ...
            end_points['pool3'] = tf.contrib.layers.max_pool2d(end_points['inception_4e'], [ 3, 3 ], scope='pool3')
            ...
            end_points['pool4'] = tf.contrib.layers.avg_pool2d(end_points['inception_5b'], [ 7, 7 ], stride = 1, scope='pool4')

            end_points['reshape'] = tf.reshape( end_points['pool4'], [-1, 1024] )

            end_points['dropout'] = tf.contrib.layers.dropout( end_points['reshape'], dropout_keep_prob, is_training = is_training )

            end_points['logits'] = tf.contrib.layers.fully_connected( end_points['dropout'], num_classes, activation_fn=None, scope='logits')
